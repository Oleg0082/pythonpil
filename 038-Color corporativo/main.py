...
# Corporate color: DarkSeaGreen
corporate_color = "#8FBC8F"

# Controls frame on the left
controls_frame = tk.Frame(ventana, bg=corporate_color)
controls_frame.grid(row=0, column=0, padx=20, pady=20, sticky="nw")

# Preview frame on the right
preview_frame = tk.Frame(ventana, bg=corporate_color)
preview_frame.grid(row=0, column=1, padx=20, pady=20, sticky="ne")

# Buttons and controls
btn_abrir = tk.Button(controls_frame, text="Selecciona la imagen de entrada", command=abrirImagen, width=30, bg=corporate_color, fg="white")
btn_abrir.grid(row=0, column=0, pady=10)

radio_negativo = tk.Radiobutton(controls_frame, text="Negativo", variable=opcion, value=1, command=aplicarCambios, bg=corporate_color, fg="white")
radio_negativo.grid(row=1, column=0, sticky="w")

radio_brillo = tk.Radiobutton(controls_frame, text="Brillo", variable=opcion, value=2, bg=corporate_color, fg="white")
radio_brillo.grid(row=2, column=0, sticky="w")

brillo_slider = tk.Scale(controls_frame, from_=-200, to=200, orient="horizontal", command=cambiaBrillo, bg=corporate_color, fg="white")
brillo_slider.grid(row=3, column=0, sticky="w")

radio_contraste = tk.Radiobutton(controls_frame, text="Contraste", variable=opcion, value=3, bg=corporate_color, fg="white")
radio_contraste.grid(row=4, column=0, sticky="w")

contraste_slider = tk.Scale(controls_frame, from_=0.1, to=4, resolution=1, orient="horizontal", command=cambiaContraste, bg=corporate_color, fg="white")
contraste_slider.grid(row=5, column=0, sticky="w")

btn_guardar = tk.Button(controls_frame, text="Selecciona la imagen de salida", command=guardarImagen, width=30, bg=corporate_color, fg="white")
btn_guardar.grid(row=6, column=0, pady=10)

btn_procesar = tk.Button(controls_frame, text="Procesar", command=procesaImagen, width=30, bg=corporate_color, fg="white")
btn_procesar.grid(row=7, column=0, pady=10)

retroalimentacion = tk.Label(controls_frame, text="", bg=corporate_color, fg="white")
retroalimentacion.grid(row=8, column=0, pady=10)

# Preview label
preview_label = tk.Label(preview_frame, bg=corporate_color)
preview_label.pack()
